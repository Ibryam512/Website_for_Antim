...
<html>
	...
	<div id="menu">
			<ul>
				...
				?>
				<li id="options"><a href="users.php" style="background-color: white;">Потребители</a></li>
				<?php
					include 'connect.php';
					if(!empty($_SESSION['ID']))
					{
		...
		<?php
			...
			while($row = $result->fetch_assoc())
			{
				...
				$pic = $row["pic"];
				$image = "pic/profilePic.png";
				if($pic != null)
				{
					$image = "pic/PROF/".$pic;
				}
				$str .= "<li class='collection-item avatar'>
				<img src='$image' alt='Профилна снимка' class='circle'>
			   	<span class='title'>$name $secName $lastName</span>
				<a href='items.php?id=$id' class='collection-item'>Всички обяви</a>
				<a href='delete.php?table=users&id=$id' class='secondary-content' style='color: red;'>Изтрий</a>
				</li>";
